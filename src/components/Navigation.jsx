  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-surface-dark border-t border-primary/20 px-6 py-4 rounded-t-[2.5rem] shadow-[0_-10px_40px_rgba(0,0,0,0.5)] max-w-md mx-auto">
      <div className="flex items-center justify-between">
        ...
        <NavLink to="/profile" className={({ isActive }) => `flex flex-col items-center gap-1 group transition-all duration-300 ${isActive ? 'scale-110' : 'hover:scale-105'}`}>
          {({ isActive }) => (
            <>
              <span className={`material-symbols-outlined transition-colors duration-300 ${isActive ? 'text-primary fill-[1]' : 'text-slate-500 group-hover:text-primary/70'}`}>person</span>
              <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-300 ${isActive ? 'text-primary' : 'text-slate-500 group-hover:text-primary/70'}`}>{t('nav.profile')}</span>
            </>
          )}
        </NavLink>
      </div>
